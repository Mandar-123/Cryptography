...
def doubleColumnarDecrypt(ct, rows, cols, key = 0):
        pt = simpleColumnarDecrypt(simpleColumnarDecrypt(ct, rows, cols, key), rows, cols, key)
        return pt
...
while(True):
	...
	if ch == '1':	
		...
		rows = input("Enter number of rows: ")
		cols = input("Enter number of columns: ")
		pt = doubleColumnarDecrypt(ct, int(rows), int(cols), key)
